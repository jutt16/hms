...
interface LeavesIndexProps extends PageProps {
    leaves: {
        data: Leave[];
        current_page: number;
        last_page: number;
...
export default function Index({ leaves, staff, filters }: LeavesIndexProps) {
    const handleDelete = (id: number) => {
        if (confirm('Are you sure you want to delete this leave request?')) {
            router.delete(`/admin/staff/leaves/${id}`);
        }
    };
        ...
        <AuthenticatedLayout title="Leave Requests">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <Link
                        href="/admin/staff/leaves/create"
                ...
                <div className="bg-white dark:bg-gray-800 shadow rounded-lg mb-6">
                    <div className="px-4 py-5 sm:p-6">
                        <form method="get" action="/admin/staff/leaves" className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                            <div>
                                        ...
                                        <tr key={leave.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div className="flex justify-end gap-2">
                                                    <Link
                                                        href={`/admin/staff/leaves/${leave.id}`}
                        ...
                        {leaves.last_page > 1 && (
                            <div className="mt-4 flex items-center justify-between">
                                <div className="flex-1 flex justify-between sm:hidden">
                                    ...
                                    {leaves.links[leaves.links.length - 1].url && (
                                        <Link
                                            href={leaves.links[leaves.links.length - 1].url}
                                ...
                                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                    <div>
                                        <p className="text-sm text-gray-700 dark:text-gray-300">
                                            Showing page <span className="font-medium">{leaves.current_page}</span> of{' '}
                                            <span className="font-medium">{leaves.last_page}</span>
                                        </p>
                                    </div>
                                    <div>
                                        <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                            {leaves.links.map((link, index) => (
                                                <Link
                                                    ...
                                                    } ${index === 0 ? 'rounded-l-md' : ''} ${index === leaves.links.length - 1 ? 'rounded-r-md' : ''}`}
                                                >
