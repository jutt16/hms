...
interface StaffIndexProps extends PageProps {
    staff: {
        data: Staff[];
        current_page: number;
        last_page: number;
...
export default function Index({ staff, filters }: StaffIndexProps) {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white dark:bg-gray-800 shadow rounded-lg mb-6">
                    <div className="px-4 py-5 sm:p-6">
                        <form method="get" action="/admin/staff/staff" className="flex gap-4">
                                    ...
                                    {staff.data.map((staffMember) => (
                                        <tr key={staffMember.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                                <div>{staffMember.user.name}</div>
                                                <div className="text-xs text-gray-400">{staffMember.user.email}</div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                                <span className="capitalize">{staffMember.employee_type.replace('_', ' ')}</span>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div className="flex justify-end gap-2">
                                                    <Link
                                                        href={`/admin/staff/staff/${staffMember.id}`}
                                                        className="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300"
                                                    >
                                                        View
                                                    </Link>
                                                    <Link
                                                        href={`/admin/staff/staff/${staffMember.id}/edit`}
                        ...
                        {staff.last_page > 1 && (
                            <div className="mt-4 flex items-center justify-between">
                                <div className="flex-1 flex justify-between sm:hidden">
                                    ...
                                    {staff.links[staff.links.length - 1].url && (
                                        <Link
                                            href={staff.links[staff.links.length - 1].url}
                                ...
                                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                    <div>
                                        <p className="text-sm text-gray-700 dark:text-gray-300">
                                            Showing page <span className="font-medium">{staff.current_page}</span> of{' '}
                                            <span className="font-medium">{staff.last_page}</span>
                                        </p>
                                    </div>
                                    <div>
                                        <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                            {staff.links.map((link, index) => (
                                                <Link
                                                    ...
                                                    } ${index === 0 ? 'rounded-l-md' : ''} ${index === staff.links.length - 1 ? 'rounded-r-md' : ''}`}
                                                >
